class UrbanMapApp {
    ...
    setTool(tool) {
        ...
        this.canvas.forEachObject(o => {
            o.selectable = tool === 'select' && !!o.isBuilding && o.floorId === this.state.currentFloorId;
            o.evented = true;
        });
    ...
    updateDimensions(obj) {
        ...
        for (let i = 0; i < vertices.length; i++) {
            ...
            const towardCenter = Math.hypot(mid.x + nx * 18 - center.x, mid.y + ny * 18 - center.y) < Math.hypot(mid.x - center.x, mid.y - center.y);
            if (towardCenter) { nx *= -1; ny *= -1; }

            const offset = Math.min(22, Math.max(10, distPx * 0.09));
    ...
    updateUrbanIndices() {
        ...
        const groundId = this.state.floors.find(f => f.isGround)?.id;
